...
| `GET /agents/activity` | `fetchAgentActivity` | → `AgentActivity` (`entities/agentusage/model.ts`). **순위는 넣지 않는다** — 횟수만 주면 화면이 센다. 둘 다 오면 어긋나는 날이 온다 |
...
**3-9. AI 엔진과 응답 형식 — 에이전트 13종이 무엇으로 도는가**

> ⚠️ **이 항목은 2026-08-08에 뒤늦게 추가했다.** §3에 8건을 적어 두고도 코드에는
> `제거 조건 = OCR 엔진·응답 형식 확정` 같은 표시가 **13종류** 있었다. 백엔드가
> 8건을 다 답해도 **에이전트 13종은 여전히 못 붙는다** — 물어본 적이 없어서 몰랐던 것이다.
> 코드의 '제거 조건'을 세어 보고서야 드러났다.

- **질문**: 아래 13개를 각각 무엇으로 처리하나. 그리고 **그 응답이 어떤 형태로 오나**

  | 무엇 | 걸린 경계 파일 | 지금 화면이 보여 주는 것 |
  |---|---|---|
  | 요약 모델 | `summary.ts` | 압축률·소요 시간·키워드 신뢰도 |
  | 번역 엔진 | `translation.ts` | 문장별 신뢰도·역번역·적용 용어집 |
  | 검토 모델 | `review.ts` | 조항 위반 후보와 근거 조항 |
  | 생성 모델(보고서) | `report.ts` | 미기재 항목 목록 |
  | STT + 생성(회의록) | `meeting.ts` | 발언자 분리·결정/담당 미정 표시 |
  | OCR 엔진 | `ocr.ts` | 줄별 신뢰도·개인정보 마스킹 |
  | Text-to-SQL | `dataquery.ts` | 생성 SQL·가정한 조건·변환 실패 표현 |
  | 지식 검색 | `knowledge.ts` | 근거 문서·유사도 |
  | 규정 검색 | `regulation.ts` | 조항 근거 |
  | 분석 엔진 | `analysis.ts` | 추이·분포·부분 결론 비율 |
  | 위험성 평가 | `safety.ts` | 위험요인과 사람 확인 지점 |
  | 문서 업로드·파싱 | `documents.ts` | 최대 크기, 파싱 실패 사유 |
  | 데이터셋 업로드·집계 | `datasets.ts` | 행 수·결측 집계 |

- **권장**: 엔진 선택은 저희가 정할 일이 아니다. 다만 **응답에 반드시 있어야 하는 것**이
  있다 — `confidence`(0~1), 못 한 것의 **목록**(빈 배열이 아니라 사유가 담긴 항목),
  그리고 처리 시간. 이것이 없으면 **이 제품의 태도가 무너진다**:
  화면마다 'AI가 못 한 것을 먼저 말한다'가 핵심인데, 서버가 안 주면 말할 것이 없다
- **안 정하면**: **에이전트 13종을 하나도 못 붙인다.** §6의 3단계가 통째로 막힌다.
  §3의 나머지 8건을 다 답해도 마찬가지다
- **정해지면**: 엔진마다 경계 함수 1~2개를 바꾼다. 화면은 손대지 않는다 —
  단, **응답에 `confidence`나 '못 한 것'이 없으면 화면에서 그 영역을 지워야 한다.**
  그때는 화면 변경이 생기므로 미리 알려 주셔야 한다

**답해 주실 분이 다릅니다** — 3-1·3-2·3-6·3-8은 백엔드, 3-3·3-4·3-5·3-7은 **보안·법무**,
3-9는 **AI 플랫폼** 쪽입니다. 한 사람에게 9건을 다 보내면 왕복이 길어집니다.

---

## 3-답. 회신란 (이대로 채워 주시면 됩니다)

권장안대로면 **`승인`만 적으면 됩니다.** 다르게 가면 `수정`에 결정 내용을,
아직이면 `보류`에 예상 시점을 적어 주십시오.

| # | 결정 | 답해 주실 분 | 승인 / 수정 / 보류 | 내용 |
|---|---|---|---|---|
| 3-1 | 인증 방식 | 백엔드 | | |
| 3-2 | 테넌시(헤더 vs 경로) | 백엔드 | | |
| 3-2-1 | 관리자의 발주처 열람 범위 | 백엔드 + 보안 | | |
| 3-3 | 보안 등급 서버 필터 | 보안 | | |
| 3-4 | 감사 로그 범위 | 보안·법무 | | |
| 3-5 | 파일 보관 기간·삭제 | 법무 | | |
| 3-6 | 동시성·상한 | 백엔드 | | |
| 3-7 | 질의 본문 보관 | 법무 | | |
| 3-8 | 오류 코드 체계 | 백엔드 | | |
| 3-9 | AI 엔진·응답 형식 13종 | AI 플랫폼 | | |

**먼저 답해 주시면 좋은 순서**: 3-2 → 3-3 → 3-9 → 3-1.
앞의 셋이 §6의 1·2·3단계를 열고, 3-1은 관리자와 상태 변경을 엽니다.
...
| 3 | 에이전트 실행 9종(`/summaries`·`/translations`·…) | 에이전트 13종 | **§3-9 AI 엔진·응답 형식**, §3-5 파일 보관, §3-6 상한 |
...
⚠️ **3단계는 백엔드만으로 안 열린다.** 에이전트 13종은 §3-9(어떤 AI 엔진을 쓰고 응답이
어떤 형태인가)가 정해져야 붙는다. 처음에 §3을 8건으로 적었을 때 이 표에도 그 의존이
빠져 있었다 — 코드의 '제거 조건' 표시를 세어 보고서야 드러났다(13종류).

### 이 결정이 여는 함수 수 — 코드에서 센 값

`shared/api`의 `TODO(api-미확정)` 표시마다 **제거 조건**이 적혀 있다. 그것을 세면
어떤 결정이 얼마를 여는지 나온다. 이 수는 `scripts/api-contract.test.ts`가 지킨다 —
새 표시가 목록에 없는 조건을 쓰면 검사가 깨진다.

| 제거 조건 | 여는 함수 |
|---|---|
| 백엔드가 제안서를 확정(§3-2 포함) | **59** |
| 백엔드가 인증·권한을 확정(§3-1·§3-3) | 12 |
| AI 엔진·응답 형식(§3-9) | **13종류** — 에이전트 13종이 여기 걸려 있다 |
| 보관 정책(§3-5) | 5 |
| 테넌시를 명시적으로 부르는 것(§3-2) | 3 |
| 그 밖(인증·검토 흐름, 요금 주기, 감사 범위 등) | 나머지 |
