...
import { blockedBy, unknownAgents } from '@entities/agentdef/model'
...
export function ScenarioBuilderPage() {
  ...
  const scenarios = useRemote(fetchScenarioDefs, [])
    ...
    <main className="min-w-0 p-4 sm:p-6">
      ...
      {scenarios.kind === 'ready' &&
        ...
        (() => {
          ...
          const broken = withState.filter((x) => x.s.enabled && (x.blocked.length > 0 || x.missing.length > 0))
          ...
          return (
            <>
              ...
              <ul aria-label="시나리오" className="mt-4 space-y-3">
                {[...broken, ...rest].map(({ s, blocked, missing }) => (
                  <li
                    key={s.id}
                    className={`rounded-xl border p-4 ${
                      blocked.length > 0 || missing.length > 0
                      ...
                      {s.steps.map((st) => {
                        ...
                        return (
                          <li key={st.order} className="text-xs">
                            <span className={`font-bold ${bad ? 'text-rose-800' : 'text-slate-800'}`}>
