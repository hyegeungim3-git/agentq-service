...
describe('SignalBell', () => {
  ...
  it('열면 신호마다 출처를 함께 보여준다', async () => {
    render(<SignalBell signals={SIGNALS} onOpen={vi.fn()} />)
    await userEvent.click(screen.getByRole('button', { name: /업무 알림/ }))
    const box = screen.getByRole('region', { name: '업무 알림' })
  ...
  it('처리 화면으로 이으면 목록이 닫힌다', async () => {
    ...
    expect(screen.queryByRole('region', { name: '업무 알림' })).not.toBeInTheDocument()
  })
