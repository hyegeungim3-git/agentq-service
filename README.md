...
| | |
|---|---|
| 사용자 포털 | 분야 선택 → **셸(일반·에이전트·보안 탭)** → 에이전트 13종 + 복합 업무 릴레이 1종 |
| 발주처 | 업무 데이터가 있는 곳은 제조(한빛정밀) 1종. 나머지 3종은 선택 불가로 표시 |
| 관리자 | 미착수 |
| 백엔드 | **미연결** — 데이터는 전부 `src/fixtures/` |
| 파일 업로드 | 형식·용량 검사와 화면은 동작. 전송은 서버 연결 후 (`docs/DECISIONS.md` D-009) |
| 검증 | lint · typecheck · 단위 · E2E · build 전부 CI 게이트 |
